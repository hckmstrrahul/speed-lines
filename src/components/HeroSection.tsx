...
const HeroSection: React.FC<HeroSectionProps> = ({ config, gridIdPrefix = "" }) => {
  const gfv = `${gridIdPrefix}grid-fade-v`;
  const gfh = `${gridIdPrefix}grid-fade-h`;
  const gm = `${gridIdPrefix}grid-mask`;
  const gp = `${gridIdPrefix}grid-pattern`;
  if (!config.showOverlay) return null;

  return (
    <>
      {/* Decorative layer — no stacking context so backdrop-blur can see through */}
        ...
        {config.showGrid && (
          ...
          >
            <defs>
              ...
              <mask id={gm}>
                <rect width="100%" height="100%" fill={`url(#${gfv})`} />
                <rect width="100%" height="100%" fill={`url(#${gfh})`} style={{ mixBlendMode: "multiply" }} />
              </mask>
              <pattern
                id={gp}
      ...
      {/* Interactive layer — sits above background, button can backdrop-blur through to speed lines */}
      <div
        className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center"
      >
        <div
          className="pointer-events-auto relative flex w-[1200px] flex-col items-center gap-[10px] px-5 text-center"
        ...
        >
          ...
          {config.overlayButtonText && (
            <a
              ...
              className="group relative mt-[4px] overflow-hidden rounded-[60px] backdrop-blur-[4px] cursor-pointer transition-all duration-300 hover:scale-[1.03] active:scale-[0.97] focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-white/40"
              style={{
                height: 64,
                border: "none",
                background:
                  "linear-gradient(93deg, rgba(255, 255, 255, 0.06) 0%, rgba(142, 144, 145, 0.08) 100%)",
